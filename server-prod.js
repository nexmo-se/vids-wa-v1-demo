...
const url = 'https://vids.vonage.com/wav1/webhooks/inbound';
...
app.post('/webhooks/inbound', (req, res) => {
  ...
  if (/\d+ ([^,]+), ([A-Z]{2}) (\d{5})/.test(text)) {
    ...
    if (reply || !/\d+ ([^,]+), ([A-Z]{2}) (\d{5})/.test(text)) {
      switch (reply) {
        ...
        case 'LEAVE':
          textToSend =
            'Sorry to see you leave. You can visit Vonage-Shopping.com to opt into the virtual assistant a$
          sendText(req, res, textToSend);
          break;
        case 'STAY':
          textToSend =
            'Fabulous. Let’s start with shirt colors. Are you looking for a light color or dark color shir$
  ...
  // res.status(200).end(); // Cannot set headers after they are sent to the client (Because in sendMessag$
});
